...
class AppointmentController extends Controller
{
    ...
    $doctor = Doctors::where('user_id',auth()->user()->id)->first();
        // dd($doctor->id);
    $appointments=Appointment::with(['patient','doctor.user'])->where('doctor_id',$doctor->id,)->where('status','checked')->orWhere('status','Pay Confirm')->orWhere('status','cancel')->paginate(5);
    ...
    {
    $doctors=User::where('role','doctor')->with('doctor')->paginate();
    return view('admin.doctorProfile',compact('doctors'));
    }
    ...
    return view('user.appointmentList',compact('appointment'));
    }
